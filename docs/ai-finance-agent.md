...
O agente utiliza a classe `ChatOpenAI` do pacote `langchain_openai`. Isso permite que possamos apontar o agente tanto para a **Groq API** (usando o modelo `openai/gpt-oss-20b` ou similar de baixíssima latência) quanto para a própria **OpenAI** (usando o `GPT-5-mini` ou similar).
